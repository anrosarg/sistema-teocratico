<x-app-layout>
    ...
    <div class="max-w-2xl mx-auto py-8">
        <form method="POST" action="{{ route('publishers.update', $publisher->id) }}" class="bg-white p-6 rounded shadow">
            @csrf
            @method('PUT')
            ...
            <div class="mb-4">
                ...
                <input type="text" name="first_name" class="w-full border rounded px-3 py-2"
                    value="{{ old('first_name', $publisher->first_name) }}" required>
            ...
            <div class="mb-4">
                ...
                <input type="text" name="last_name" class="w-full border rounded px-3 py-2"
                    value="{{ old('last_name', $publisher->last_name) }}" required>
            ...
            <div class="mb-4">
                ...
                <select name="group_id" class="w-full border rounded px-3 py-2" required>
                    ...
                    @foreach($grupos as $grupo)
                        <option value="{{ $grupo->id }}" @selected(old('group_id', $publisher->group_id) == $grupo->id)>
                            {{ $grupo->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-4">
                ...
                <select name="circuit_id" class="w-full border rounded px-3 py-2" required>
                    ...
                    @foreach($circuitos as $circuito)
                        <option value="{{ $circuito->id }}" @selected(old('circuit_id', $publisher->circuit_id) == $circuito->id)>
                            {{ $circuito->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-4">
                ...
                <select name="congregation_id" class="w-full border rounded px-3 py-2" required>
                    ...
                    @foreach($congregaciones as $congregacion)
                        <option value="{{ $congregacion->id }}" @selected(old('congregation_id', $publisher->congregation_id) == $congregacion->id)>
                            {{ $congregacion->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-4">
                ...
                <select name="status" class="w-full border rounded px-3 py-2" required>
                    ...
                    @foreach($estados as $estado)
                        <option value="{{ $estado }}" @selected(old('status', $publisher->status) == $estado)>
                            {{ ucfirst($estado) }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-4">
                ...
                <select name="condition" class="w-full border rounded px-3 py-2" required>
                    ...
                    @foreach($condiciones as $condicion)
                        <option value="{{ $condicion }}" @selected(old('condition', $publisher->condition) == $condicion)>
                            {{ ucfirst($condicion) }}
                        </option>
                    @endforeach
                </select>
                ...
                @foreach($privilegios as $privilegio)
                    <label class="inline-flex items-center mr-4 mb-2">
                        <input type="checkbox" name="privilegios[]" value="{{ $privilegio->id }}"
                            ...
                            {{ in_array($privilegio->id, old('privilegios', $publisher->privileges->pluck('id')->toArray())) ? 'checked' : '' }}>
                ...
                @foreach($asignaciones as $asignacion)
                    <label class="inline-flex items-center mr-4 mb-2">
                        <input type="checkbox" name="asignaciones[]" value="{{ $asignacion->id }}"
                            ...
                            {{ in_array($asignacion->id, old('asignaciones', $publisher->assignments->pluck('id')->toArray())) ? 'checked' : '' }}>
            ...
            <div class="flex justify-end">
                <a href="{{ route('publishers.index') }}" class="bg-gray-300 hover:bg-gray-400 text-gray-800 font-semibold py-2 px-4 rounded mr-2">Cancelar</a>
                <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2 px-4 rounded">Actualizar</button>
            </div>
        </form>
    </div>
</x-app-layout>
