...
<head>
    <meta charset="UTF-8">
    <title>Crear reunión fin de semana</title>
...
<body class="bg-gray-100 min-h-screen text-gray-800">
    ...
    <main class="max-w-2xl mx-auto py-8 px-6">
        <h1 class="text-3xl font-bold mb-8">Agregar reunión fin de semana</h1>

        {{-- Formulario --}}
        <form action="{{ route('reunion-fin-de-semana.store') }}" method="POST" class="bg-white p-8 rounded-lg shadow-md space-y-6">
            @csrf

            {{-- Fecha --}}
            <div>
                <label for="fecha" class="block font-semibold mb-2 text-lg">Fecha</label>
                <input type="date" name="fecha" id="fecha" class="w-full border rounded px-4 py-3 text-base" required>
            </div>
            ...
            <div>
                ...
                <input type="text" name="orador" id="orador" class="w-full border rounded px-4 py-3 text-base" required>
            </div>

            {{-- Congregación --}}
            <div>
                <label for="congregacion" class="block font-semibold mb-2 text-lg">Congregación</label>
                <input type="text" name="congregacion" id="congregacion" class="w-full border rounded px-4 py-3 text-base" required>
            </div>

            {{-- Tema --}}
            <div>
                <label for="tema" class="block font-semibold mb-2 text-lg">Tema del discurso</label>
                <input type="text" name="tema" id="tema" class="w-full border rounded px-4 py-3 text-base" maxlength="256" required>
            </div>

            {{-- La Atalaya --}}
            <div>
                <label for="la_atalaya" class="block font-semibold mb-2 text-lg">La Atalaya (lector)</label>
                <input type="text" name="la_atalaya" id="la_atalaya" class="w-full border rounded px-4 py-3 text-base" required>
            </div>
            ...
            <div class="flex justify-end space-x-4 mt-8">
                ...
                <button type="submit" class="px-5 py-3 bg-blue-600 text-white text-base rounded hover:bg-blue-700 transition">Guardar</button>
            </div>
        </form>
    </main>
